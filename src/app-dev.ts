...
/* ── MAINTENANCE MODE ────────────────────────────────────────────────────────
   ...
   Admins always pass through. Testers visit:
   https://krishnacontest.org/?__bypass=<MAINTENANCE_BYPASS_TOKEN>
   to get an 8-hour bypass cookie.
──────────────────────────────────────────────────────────────────────────── */
if (process.env.MAINTENANCE_MODE === 'true') {
  const BYPASS_TOKEN = process.env.MAINTENANCE_BYPASS_TOKEN || 'iskcon-dev-2026';

  app.use((req: any, res: any, next: any) => {
    // Set bypass cookie when token is in URL, then redirect to clean URL
    if (req.query.__bypass === BYPASS_TOKEN) {
      res.cookie('maintenance_bypass', BYPASS_TOKEN, {
        httpOnly: true,
        maxAge: 8 * 60 * 60 * 1000, // 8 hours
        path: '/',
      });
      const clean = req.path + (Object.keys(req.query).filter(k => k !== '__bypass').length
        ? '?' + Object.entries(req.query).filter(([k]) => k !== '__bypass').map(([k,v]) => `${k}=${v}`).join('&')
        : '');
    ...
    // Always allow: admin, static assets, webhooks, health check, bypass
    const alwaysAllow =
      hasBypass ||
      ...
      req.path === '/healthz' ||
      req.path.includes('/webhook');  // never block webhook callbacks
