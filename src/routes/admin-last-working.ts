...
router.post(
  ...
  async (req: any, res) => {
    ...
    try {
      ...
      if (!shipmentKey || !phoneKey || !trackingKey) {
        fs.unlink(req.file.path, () => {});
        return res.redirect(
          "/admin/shipments?errorMsg=" +
            encodeURIComponent(
              "Missing required columns. Expected shipment_id, phone and tracking_id."
            )
        );
      }
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const shipmentId = normalizeCell(row[shipmentKey]);
        const phone = normalizeCell(row[phoneKey]);
        const trackingId = normalizeCell(row[trackingKey]);
      ...
      for (const row of dbQ.rows) {
        if (!dbMap.has(String(row.id))) {
          dbMap.set(String(row.id), row);
        }
      }

      const nonEmptyTrackingIds = Array.from(
        new Set(
          parsedRows
            .map(r => normalizeCell(r.trackingId))
            .filter(Boolean)
            .map(normalizeCompare)
        )
      );

      const existingTrackingQ = nonEmptyTrackingIds.length
        ? await pool.query(
            `
            SELECT id, tracking_id
            FROM shipments
      ...
      const trackingToShipmentIds = new Map<string, string[]>();
      for (const row of existingTrackingQ.rows) {
        const k = normalizeCompare(row.tracking_id);
        const arr = trackingToShipmentIds.get(k) || [];
        arr.push(String(row.id));
        trackingToShipmentIds.set(k, arr);
      }
      ...
      for (const r of parsedRows) {
        ...
        if (normalizeCompare(dbRow.status) !== "packed") {
          rejectedRows.push({
            ...r.raw,
            ERROR_REASON: "Shipment is not in packed status",
            ERROR_ROW_NO: r.rowNo,
          });
          continue;
        }

        const dbPhone = normalizeCompare(dbRow.recipient_phone || dbRow.user_phone || "");
        const filePhone = normalizeCompare(r.phone);

        if (!dbPhone || dbPhone !== filePhone) {
          rejectedRows.push({
            ...r.raw,
            ERROR_REASON: "Phone number does not match shipment record",
            ERROR_ROW_NO: r.rowNo,
          });
          continue;
        }

        const uploadedTracking = normalizeCell(r.trackingId);
        const currentTracking = normalizeCell(dbRow.tracking_id);

        if (uploadedTracking) {
          const usedBy = trackingToShipmentIds.get(normalizeCompare(uploadedTracking)) || [];
          const usedByAnotherShipment = usedBy.some((id) => id !== r.shipmentId);
        ...
        // empty tracking in upload is allowed -> retain existing one
        validRows.push({
          shipmentId: r.shipmentId,
          trackingId: uploadedTracking || currentTracking,
        });
      }
        ...
        for (const r of validRows) {
          await client.query(
            ...
            WHERE id = $2
              ...
              AND LOWER(COALESCE(delivery_mode, '')) = 'home_delivery'
            `,
            [r.trackingId || null, r.shipmentId]
          );
        }
      ...
      if (rejectedRows.length) {
        ...
        const tmpName = `rejected_dispatched_${Date.now()}.xlsx`;
        tmpRejectedPath = path.join(process.cwd(), "tmp", tmpName);
        XLSX.writeFile(rejWb, tmpRejectedPath);

        return res.redirect(
          "/admin/shipments?okMsg=" +
            encodeURIComponent(
              `Dispatched import completed. ${validRows.length} rows updated. ${rejectedRows.length} rows rejected.`
            ) +
          "&errorMsg=" +
            encodeURIComponent("Some rows were rejected. Download and review the rejected file.") +
          "&rejectedFile=" +
            encodeURIComponent(tmpName)
        );
      }
      ...
      return res.redirect(
        "/admin/shipments?okMsg=" +
          encodeURIComponent(
            `Dispatched import completed. ${validRows.length} rows updated. 0 rows rejected.`
          )
      );
    } catch (e) {
...
router.get("/admin/shipments/download-packed", authMiddleware, adminMiddleware, async (req: any, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="shipments_packed_home_delivery_${stamp}.csv"`
    );
