...
function buildShipmentFileName(prefix: string, ext = "csv") {
  const now = new Date();
...
function shipmentHeaderRow(extraFlag: "" | "PACKED_OR_NOT" | "DELIVERED_OR_NOT" | "START_AGAIN" = "") {
  const headers = [
    "SHIPMENT_ID",
    "PAYMENT_ID",
    "USER_ID",
...
function shipmentCsvText(
  rows: any[],
  statusOverride?: string,
  extraFlag: "" | "PACKED_OR_NOT" | "DELIVERED_OR_NOT" | "START_AGAIN" = ""
) {
  ...
  return [
    ...
    ...rows.map((r: any) => {
      const baseRow = [
        ...
        csvEscape(statusOverride || r.status),
      ];

      if (extraFlag) baseRow.push(csvEscape(""));
...
if (dateFrom && dateTo && dateFrom > dateTo) {
  return res.redirect(
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="shipments_dispatched_home_delivery_${stamp}.csv"`
    );
   ...
   console.log("Called downl;oad pending:");
    ...
    const csv = [
      ...
      ...rows.map((r: any) => [
        ...
        csvEscape(r.courier_mode),
        csvEscape(r.delivery_mode),
        csvEscape(SHIPMENT_STATUS.PENDING),
      ].join(","))
    ].join("\n");
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="shipments_pending_download_${stamp}.csv"`
    );
  ...
  async (req: any, res) => {
    ...
    try {
      ...
      const redirectUrl =
          ...
          "&okMsg=" + encodeURIComponent(`Under packing CSV update completed. Accepted: ${accepted}.`) +
          (rejected.length
        ...
        return jsonOk(res, {
          message: `Under packing CSV update completed.`,
...
router.get("/admin/shipments/download-status", authMiddleware, adminMiddleware, async (req: any, res) => {
  try {
    ...
    const csv = shipmentCsvText(rows, undefined, extraFlag as any);
  ...
  async (req: any, res) => {
    try {
      ...
      if (!req.file?.path) {
        ...
        return jsonBad(res, "Please choose a CSV/XLSX file.");
      }
      ...
      if (!(fromStatus === "pending" && toStatus === "under_packing")) {
        ...
        if (!isAjax) {
          return res.redirect(
            "/admin/shipments?delivery_mode=home_delivery&errorMsg=" +
              encodeURIComponent("Unsupported CSV transition.")
          );
        }
        return jsonBad(res, "Unsupported CSV transition.");
      }
      ...
      if (duplicateTrackingSet.size > 0) {
        ...
        return jsonOk(res, {
          message:
            "Pending CSV update cancelled. Duplicate TRACKING_ID found in uploaded file, so no rows were updated.",
          acceptedCount: 0,
      ...
      return jsonOk(res, {
        message: "Pending CSV update completed.",
        acceptedCount: validRows.length,
        rejectedCount: rejectedRows.length,
        rejectedFile,
        redirectUrl,
