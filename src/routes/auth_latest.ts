...
/**
 * TEST MODE gate:
 * Allow only admins OR the configured test email.
 */
function isTestModeBlocked(user: { email?: string; role?: string }) {
  if (process.env.TEST_MODE !== "true") return false;

  const testEmail = normEmail(process.env.TEST_EMAIL || "iskcon.contest.test@gmail.com");
  const email = normEmail(user.email || "");
  const role = String(user.role || "").toLowerCase();

  const allowed = role === "admin" || email === testEmail;
  return !allowed;
}
...
router.get("/register", (_req: Request, res: Response) => {
  // Optional: show message on register page itself in test mode
...
router.post(
  ...
  async (req: Request, res: Response) => {
    // ✅ Block registrations in TEST MODE
    if (process.env.TEST_MODE === "true") {
      return renderTestingMode(
        res,
        "🚧 Website is currently under testing. New registrations are temporarily disabled."
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      // ✅ fetch role + email for test-mode gating
      ...
      // ✅ TEST MODE: allow only admin or test email
      if (isTestModeBlocked(user)) {
        return renderTestingMode(
          res,
          "🚧 Website is under testing. Only admin and approved test users can login."
        );
      }
...
router.get(
  ...
  async (req: any, res: any) => {
    ...
    try {
      // ✅ Fetch user for test-mode gating
      const u = await pool.query(`SELECT email, role FROM users WHERE id=$1`, [userId]);
      const user = u.rows[0] || {};
