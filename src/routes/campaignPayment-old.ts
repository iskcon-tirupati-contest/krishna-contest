...
// Phones that mean "no real phone provided" — Razorpay sends 9999999999 as placeholder
...
async function createUser(params: {
  ...
  const q = await pool.query(
    ...
    [params.name, params.phone, params.phoneLocked, params.email]
  );
...
*/
   ...
   //   console.log("CAPTURE STATUS NOT MATCHING ",verifiedStatus);
    ...
    const rawPhone = normPhone(mobileNoRaw);
    ...
    const phone = hasRealPhone
      ? rawPhone
      : `TEMP_${verifiedPaymentId.slice(-6)}`;

    // Shipment blocked from dispatch until real phone is confirmed
     ...
     //  console.log("REACJED 2 =================================");
      ...
      if (!user) {
        ...
        user = await createUser({
          ...
          phone,
          email: emailToSave,
          phoneLocked: hasRealPhone, // temp phones stay unlocked so they can be updated later
        });
      } else {
