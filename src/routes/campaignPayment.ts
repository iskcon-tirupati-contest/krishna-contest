...
const EXPECTED_AMOUNT_RUPEES = 1;
...
router.post("/payment/campaign/ramayana/webhook", async (req: any, res) => {
  try {
    ...
    const bookLanguage = pickNote(notes, "book_language") || "English";
    ...
    const ageCategory = normalizeAgeCategory(ageRaw) || "above-25";
    const deliveryMode = normalizeDeliveryMode(deliveryModeRaw) || "home_delivery";
    ...
    try {
      ...
      // CRITICAL FIX: idempotent payment_sessions insert
      const paymentSessionPaymentId = verifiedOrderId || verifiedPaymentId;
      ...
      const paymentSessionQ = await pool.query(
        `INSERT INTO payment_sessions (user_id, payment_id, amount, status)
         ...
         RETURNING id`,
        [user.id, paymentSessionPaymentId, EXPECTED_AMOUNT_RUPEES]
      );
      ...
      const orderQ = await pool.query(
        ...
        [
          ...
          internalPaymentId,
          ageCategory,
          deliveryMode === "donation" ? "donation" : "book",
          fullName,
          deliveryMode === "donation" ? null : contest.default_book_title,
          paymentSessionId,
        ]
      );
