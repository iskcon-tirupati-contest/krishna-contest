...
router.post("/join/:category", async (req: any, res) => {
  ...
  if (deliveryMode !== "donate") {
    for (let i = 0; i < contestQ.rows.length; i++) {
      ...
      if (!(availableBookLanguages[b] || []).includes(l)) {
        return res.send("Invalid book-language");
      }
    }
  }
  ...
  try {
    ...
    // orders aligned with checkout.ts
    for (let i = 0; i < contestQ.rows.length; i++) {
      ...
      const ins = await pool.query(
        ...
        [
          ...
          c.id,
//          Number(c.price || 0),
          process.env.DIRECT_REG_TEST_PRICE ? Number(process.env.DIRECT_REG_TEST_PRICE) : Number(c.price || 0),
          paymentId,
          ageCategory,
    ...
    if (deliveryMode !== "donate") {
      ...
      for (let i = 0; i < orderIds.length; i++) {
        await pool.query(
          `INSERT INTO shipment_items
           (shipment_id, order_id, book_title, book_language)
           VALUES ($1,$2,$3,$4)`,
      ...
      if (category === "combo" && ssrLanguage) {
        await pool.query(
          `INSERT INTO shipment_bonus_items
           (shipment_id, book_title, book_language, quantity)
           VALUES ($1,$2,$3,1)`,
